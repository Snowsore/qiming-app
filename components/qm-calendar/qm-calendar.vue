	...
	<view class="calendarpage">
		...
		<view class="grace-btdialog-shade">
			<view class="pickerView dialog" v-show="isPicker">
				...
				<picker-view v-show="lunar != 'sizhu'" :indicator-style="indicatorStyle" :value="value" @change="bindChange" style="width: 100%; height: 328rpx">
					<picker-view-column>
						<view class="item year" v-for="(item, index) in years" :key="index" :style="{ fontWeight: index == firstSelectedIndex ? 'bold' : '' }">
							{{ item.text }}
						</view>
					</picker-view-column>
					<picker-view-column>
	...
	methods: {
		...
		init: function () {
			...
			if (lunar == 'solar') {
				...
				let saveTurn = calendarFormatter.lunar2solar(userStringArr[3], Number(userStringArr[4]), Number(userStringArr[5]), this.isRunMonthBoolean);
		...
		tabCalendar: function (tab) {
			this.lunar = tab;
			if (tab == 'lunar') {
				let saveTurn = calendarFormatter.solar2lunar(this.years[this.value[0]].value, this.months[this.value[1]].value, this.days[this.value[2]].value);
				if (saveTurn.IMonthCn.indexOf('闰') != -1) {
					this.isRunMonthBoolean = true;
				} else {
					this.isRunMonthBoolean = false;
		...
		confirm: function () {
			if (this.value[1] == '闰正月') {
				this.value[1] = 1;
			} else if (this.value[1] == '闰二月') {
				this.value[1] = 2;
			} else if (this.value[1] == '闰三月') {
				this.value[1] = 3;
			} else if (this.value[1] == '闰四月') {
				this.value[1] = 4;
			} else if (this.value[1] == '闰五月') {
				this.value[1] = 5;
			} else if (this.value[1] == '闰六月') {
				this.value[1] = 6;
			} else if (this.value[1] == '闰七月') {
				this.value[1] = 7;
			} else if (this.value[1] == '闰八月') {
				this.value[1] = 8;
			} else if (this.value[1] == '闰九月') {
				this.value[1] = 9;
			} else if (this.value[1] == '闰十月') {
				this.value[1] = 10;
			} else if (this.value[1] == '闰冬月') {
				this.value[1] = 11;
			} else if (this.value[1] == '闰腊月') {
				this.value[1] = 12;
			}
			...
			this.outterApi = [this.years[this.value[0]].value, this.months[this.value[1]].value, this.days[this.value[2]].value];
			this.initFineIndex = '';
			...
			if (this.isRunMonthBoolean == true) {
				mutiColumnPick +=
					...
					'|' +
					(this.value[3] >= 0 ? this.hours[this.value[3]].value : '-1') +
					'|' +
					'-1' +
					'|是';
			}
